%% =============================================================================
%%  reliable_event_handler_watcher.erl -
%%
...
%% @end
%% -----------------------------------------------------------------------------
-module(reliable_event_handler_watcher).
...
handle_info(Info, State) ->
    ?LOG_DEBUG("Unexpected message, message=~p", [Info]),
    {noreply, State}.
...
add_sup_handler(Manager, Handler, Args) ->
    case reliable_event_manager:add_sup_handler(Manager, Handler, Args) of
        ok ->
            ok;
        {error, {fatal, Reason}} ->
            ?LOG_ERROR(
        ...
        {error, Reason} ->
            ...
            _ = erlang:send_after(5000, self(), add_sup_handler),
            ok
    end.
